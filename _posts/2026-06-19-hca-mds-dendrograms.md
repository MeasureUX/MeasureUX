...
## Concept Check: HCA as a Puzzle

Think of each card as a puzzle piece.

In a card sorting study, participants give us clues about which pieces seem to fit together. If many participants place two cards in similar categories, those cards are probably related in the users' mental model.

HCA helps us organize these puzzle pieces into possible groups.

This is a helpful way to think about card sorting analysis:

- Each card starts as its own separate piece.
- Similar cards are gradually joined together.
- Small groups are then joined into larger groups.
- The final result is a tree-like structure called a dendrogram.

HCA does not make the final design decision for you.

It helps you see possible groupings that you can interpret as a UX researcher.
...
A reference line can be drawn across the dendrogram. Where that line crosses the branches determines the number of clusters.
